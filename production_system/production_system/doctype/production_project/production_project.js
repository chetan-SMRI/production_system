...
frappe.ui.form.on("Production Project", {
	refresh(frm) {
  frm.add_custom_button("Create a Task", () => {
    const milestone_rows = Array.isArray(frm.doc.milestones) ? frm.doc.milestones : [];
    const milestone_options = milestone_rows.map(r => (r.milestone_name || r.name || "").toString());
  
    const d = new frappe.ui.Dialog({
      title: __("Create Task"),
      fields: [
        { fieldtype: "Data", fieldname: "task_subject", label: __("Task Subject"), reqd: 1 },
        { fieldtype: "Date", fieldname: "start_date", label: __("Start Date"), reqd: 1 },
        { fieldtype: "Date", fieldname: "due_date", label: __("Due Date"), reqd: 1 },
        { fieldtype: "Link", options: "User", fieldname: "assigned_to", label: __("Assign To") },
        {
          fieldtype: "Select",
          fieldname: "milestone",
          label: __("Milestone"),
          options: milestone_options.length ? milestone_options : ["No milestones available"],
          reqd: 1
        }
      ],
      ...
      primary_action(values) {
        ...
        if (!values.start_date || !values.due_date) return frappe.msgprint(__("Please enter both start and due dates."));
        if (frappe.datetime.get_diff(values.due_date, values.start_date) < 0) return frappe.msgprint(__("Due Date must be the same or after Start Date."));
        if (!values.milestone || values.milestone === "No milestones available") return frappe.msgprint(__("Please select a milestone."));
        ...
        frappe.call({
          ...
          callback: function(r) {
            if (!r.exc && r.message && r.message.ok) {
              ...
              const msg = (r.message && r.message.message) || __("Could not create task");
  ...
  frm.add_custom_button("Delete a Task", () => {
	...
	generate_item_tasks(frm) {
		...
		frappe.msgprint({
			...
			message: __(
				"Are you sure you want to continue and list if finalized?"
			),
			primary_action: {
				label: __("Confirm"),
				action(values) {
...
function open_delete_direct_task_dialog(frm) {
  ...
  const d = new frappe.ui.Dialog({
    ...
    primary_action(values) {
      ...
      frappe.confirm(
        __("Delete task {0} ? This will remove the link from the project and delete the task.", [task_name]),
        function() {
          frappe.call({
            ...
            callback: function(r) {
              if (!r.exc && r.message && r.message.ok) {
                ...
                const msg = (r.message && r.message.message) || __("Could not delete task");
                frappe.msgprint({ title: __("Error"), message: msg, indicator: "red" });
              }
            }
          });
